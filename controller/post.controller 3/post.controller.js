  ...
  try {
    ...
    if (!media_url) {
        ...
        await sharp(media_url)
          .resize(400, 400) // set width & height
          .toFile(resizedMediaPath); // save the resized image to a new file
...
export const profileposts = async (request, response, next) => {
  try {
    ...
    if (!user_id) {
      return response
        .status(404)
        .json({ error: "Sign in for view the profile" });
    }
    if (!profileData) {
